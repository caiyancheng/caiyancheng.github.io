...
df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'lpips_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'lpips_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_final.json'), 'w') as fp:
