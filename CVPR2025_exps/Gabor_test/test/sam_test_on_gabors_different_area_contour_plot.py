...
save_root_path = '../test/new_data_logs/sam/different_area'
...
for ppd_index in tqdm(range(len(ppd_list))):
        ...
        for R_index in range(len(R_list)):
            ...
            for contrast_index in range(len(contrast_list)):
                ...
                df.to_csv(os.path.join(save_root_path,
                                       f'sam_test_on_gabors_different_area_contour_plot_ppd_{ppd_number}_temporary.csv'),
                          index=False)
    ...
    df = pd.DataFrame(csv_data)
    df.to_csv(
        os.path.join(save_root_path, f'sam_test_on_gabors_different_area_contour_plot_ppd_{ppd_number}_final.csv'),
        index=False)
    with open(os.path.join(save_root_path, f'sam_test_on_gabors_different_area_contour_plot_ppd_{ppd_number}_final.json'),
            'w') as fp:
