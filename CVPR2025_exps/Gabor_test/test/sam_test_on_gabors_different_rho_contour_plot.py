...
for ppd_index in tqdm(range(len(ppd_list))):
        ...
        for rho_index in range(len(rho_list)):
            ...
            for contrast_index in range(len(contrast_list)):
                ...
                df.to_csv(os.path.join(save_root_path,
                                       f'sam_test_on_gabors_different_rho_contour_plot_ppd_{ppd_number}_temporary.csv'),
                          index=False)
    ...
    df = pd.DataFrame(csv_data)
    df.to_csv(
        os.path.join(save_root_path, f'sam_test_on_gabors_different_rho_contour_plot_ppd_{ppd_number}_final.csv'),
        index=False)
    with open(
            os.path.join(save_root_path,
                         f'sam_test_on_gabors_different_rho_contour_plot_ppd_{ppd_number}_final.json'),
            'w') as fp:
