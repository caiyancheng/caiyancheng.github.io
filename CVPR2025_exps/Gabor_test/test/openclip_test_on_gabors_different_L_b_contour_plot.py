...
save_root_path = 'new_data_logs/openclip/different_luminance'
...
if not test_continue:
    ...
    df = pd.read_csv(os.path.join(save_root_path, f'openclip_test_on_gabors_different_luminance_contour_plot_ppd_{default_ppd}_temporary.csv'))
        ...
        for contrast_index in range(len(contrast_list)):
            ...
            if test_continue:
                check_df = df[(df['clip_model_name'] == clip_model_name) & (df['clip_model_trainset'] == clip_model_trainset)
                              & (abs(df['L_b'] - L_b_value) < 1e-5) & (abs(df['contrast'] - contrast_value) < 1e-5)]
                if len(check_df) == 1:
                    plot_L_b_matrix[L_b_index, contrast_index] = L_b_value
                    plot_contrast_matrix[L_b_index, contrast_index] = contrast_value
                    plot_final_feature_L1_similarity_matrix[L_b_index, contrast_index] = check_df['final_feature_L1_similarity']
                    plot_final_feature_L2_similarity_matrix[L_b_index, contrast_index] = check_df['final_feature_L2_similarity']
                    plot_final_feature_cos_similarity_matrix[L_b_index, contrast_index] = check_df['final_feature_cos_similarity']
                    continue
                elif len(check_df) > 1:
            ...
            df.to_csv(os.path.join(save_root_path,
                                   f'openclip_test_on_gabors_different_luminance_contour_plot_ppd_{default_ppd}_temporary.csv'),
                      index=False)
...
df = pd.DataFrame(csv_data)
df.to_csv(
    os.path.join(save_root_path, f'openclip_test_on_gabors_different_luminance_contour_plot_ppd_{default_ppd}_final.csv'),
    index=False)
with open(
        os.path.join(save_root_path, f'openclip_test_on_gabors_different_luminance_contour_plot_ppd_{default_ppd}_final.json'),
        'w') as fp:
