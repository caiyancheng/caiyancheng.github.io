...
for backbone_name in tqdm(all_backbone_list):
    ...
    for rho_index in range(len(rho_list)):
        ...
        for contrast_index in range(len(contrast_list)):
            ...
            df.to_csv(os.path.join(save_root_path, f'dino_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_temporary.csv'), index=False)
...
df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'dino_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_final.csv'), index=False)
with open(os.path.join(save_root_path, f'dino_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_final.json'), 'w') as fp:
