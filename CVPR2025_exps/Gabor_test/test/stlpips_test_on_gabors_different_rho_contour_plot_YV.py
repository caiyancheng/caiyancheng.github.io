...
save_root_path = 'new_data_logs/stlpips/different_rho_YV'
...
for rho_index in tqdm(range(len(rho_list))):
    ...
    for contrast_index in range(len(contrast_list)):
        ...
        df.to_csv(os.path.join(save_root_path, f'stlpips_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_temporary.csv'),
                  index=False)
...
df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'stlpips_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_final.csv'), index=False)
with open(os.path.join(save_root_path, f'stlpips_test_on_gabors_different_rho_contour_plot_ppd_{default_ppd}_YV_final.json'), 'w') as fp:
