...
for backbone_name in tqdm(all_backbone_list):
    ...
    for L_b_index in range(len(L_b_list)):
        ...
        for contrast_index in range(len(contrast_list)):
            ...
            df.to_csv(os.path.join(save_root_path, f'dinov2_test_on_gabors_different_L_b_contour_plot_ppd_{default_ppd}_temporary.csv'), index=False)
...
df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'dinov2_test_on_gabors_different_L_b_contour_plot_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'dinov2_test_on_gabors_different_L_b_contour_plot_ppd_{default_ppd}_final.json'), 'w') as fp:
