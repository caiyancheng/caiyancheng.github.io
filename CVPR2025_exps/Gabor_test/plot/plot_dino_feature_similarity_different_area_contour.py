...
save_root_path = 'contour_plots_arc_scale/dino/different_area'
os.makedirs(save_root_path, exist_ok=True)

json_data_path = rf'../test/new_data_logs/dino/different_area/dino_test_on_gabors_different_area_contour_plot_ppd_{ppd}_final.json'
...
for backbone_index in tqdm(range(len(backbone_name_list))):
    ...
    plot_area_matrix = np.array(plot_area_matrix_list[backbone_index])
    ...
    if backbone_name.startswith('dino_vit'):
        ...
        plot_figure_data_matrix_list = [plot_final_feature_L1_similarity_matrix / json_data['max_L1'][backbone_index],
                                        ...
                                        np.arccos(plot_final_feature_cos_similarity_matrix) / np.arccos(json_data['min_cos'][backbone_index]),
                                        plot_intermediate_feature_L1_similarity_matrix / json_data['max_L1'][backbone_index],
                                        plot_intermediate_feature_L2_similarity_matrix / json_data['max_L2'][backbone_index],
                                        np.arccos(plot_intermediate_feature_cos_similarity_matrix) / np.arccos(-1),
                                        np.arccos(plot_intermediate_feature_cos_similarity_matrix) / np.arccos(json_data['min_cos'][backbone_index])]
