...
json_data_path = rf'../test/new_data_logs/cvvdp/different_area/cvvdp_test_on_gabors_different_area_contour_plot_ppd_{ppd}_final_aim.json'
...
Y_Spearman_Score = []
for area_index in range(len(Spearman_matrix_JOD)):
    X_Spearman_multiplier += multiplier_list
    Y_Spearman_Score += Spearman_matrix_JOD[area_index]
correlation, p_value = spearmanr(X_Spearman_multiplier, Y_Spearman_Score)
print("Spearman Correlation:", round(correlation, 4))

plt.figure(figsize=(5, 3.5), dpi=300)
plt.contourf(plot_area_matrix, 1 / plot_contrast_matrix, plot_JOD_score_matrix,
             levels=20, cmap='rainbow', alpha=0.3)
contours = plt.contour(plot_area_matrix, 1 / plot_contrast_matrix, plot_JOD_score_matrix,
