...
json_data_path = rf'../test/new_data_logs/openclip/different_rho/openclip_test_on_gabors_different_rho_contour_plot_ppd_{ppd}_final_aim.json'
...
for clip_model_index in tqdm(range(len(clip_model_name_list))):
    ...
    compute_json_data[clip_model_name + ' - ' + clip_model_trainset] = {'RMSE': log_RMSE_loss, 'Pearson': correlation}
    ...
    plt.close()
with open(os.path.join(save_root_path, 'RMSE_Pearson.json'), 'w') as fp:
    json.dump(compute_json_data, fp)