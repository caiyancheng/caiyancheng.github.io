...
save_root_path = 'new_contour_plots/baseline/different_rho_RG'
os.makedirs(save_root_path, exist_ok=True)

json_data_path = rf'../test/new_data_logs/baseline/different_rho_RG/baseline_test_on_gabors_different_rho_contour_plot_ppd_{ppd}_RG_final_aim.json'
with open(json_data_path, 'r') as fp:
    plot_json_data = json.load(fp)
aim_cos_similarity = plot_json_data['aim_cos_similarity']
rho_RG_list = plot_json_data['rho_RG_list']
...
castleCSF_result_rho_RG_list = castleCSF_result_data['rho_list']
...
min_Y_rho_RG_list = []
for rho_index, rho_value in enumerate(rho_RG_list):
    ...
    if y_values_at_target_x:
        ...
        min_y_value = 1
    min_Y_rho_RG_list.append(min_y_value)
min_Y_rho_array = np.array(min_Y_rho_RG_list)
gt_f_interp = interp1d(castleCSF_result_rho_RG_list, castleCSF_result_sensitivity_list, kind='linear', fill_value="extrapolate")
gt_Y_array = gt_f_interp(rho_RG_list)
...
plt.plot(castleCSF_result_rho_RG_list, castleCSF_result_sensitivity_list, 'k', linestyle='--', linewidth=2,
         label='castleCSF (RG)')
