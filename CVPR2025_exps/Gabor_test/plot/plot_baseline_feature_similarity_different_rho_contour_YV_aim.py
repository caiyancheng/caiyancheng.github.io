...
save_root_path = 'new_contour_plots/baseline/different_rho_YV'
os.makedirs(save_root_path, exist_ok=True)

json_data_path = rf'../test/new_data_logs/baseline/different_rho_YV/baseline_test_on_gabors_different_rho_contour_plot_ppd_{ppd}_YV_final_aim.json'
with open(json_data_path, 'r') as fp:
    plot_json_data = json.load(fp)
aim_cos_similarity = plot_json_data['aim_cos_similarity']
rho_YV_list = plot_json_data['rho_YV_list']
multiplier_list = plot_json_data['multiplier_list']
Spearman_matrix_cos = plot_json_data['Spearman_matrix_cos']
...
castleCSF_result_rho_YV_list = castleCSF_result_data['rho_list']
...
plt.contourf(plot_rho_matrix, 1 / plot_contrast_matrix, plot_cos_similarity_matrix,
             levels=20, cmap='rainbow', alpha=0.3)
contours = plt.contour(plot_rho_matrix, 1 / plot_contrast_matrix, plot_cos_similarity_matrix,
...
min_Y_rho_YV_list = []
for rho_index, rho_value in enumerate(rho_YV_list):
    ...
    if y_values_at_target_x:
        ...
        min_y_value = 1
    min_Y_rho_YV_list.append(min_y_value)
min_Y_rho_array = np.array(min_Y_rho_YV_list)
gt_f_interp = interp1d(castleCSF_result_rho_YV_list, castleCSF_result_sensitivity_list, kind='linear', fill_value="extrapolate")
gt_Y_array = gt_f_interp(rho_YV_list)
...
plt.plot(castleCSF_result_rho_YV_list, castleCSF_result_sensitivity_list, 'k', linestyle='--', linewidth=2,
         label='castleCSF (YV)')
