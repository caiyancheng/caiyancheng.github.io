...
save_root_path = 'new_contour_plots/sam/different_luminance'
os.makedirs(save_root_path, exist_ok=True)

for ppd_index in range(len(ppd_list)):
    ppd_number = ppd_list[ppd_index]
    json_data_path = rf'../test/new_data_logs/sam/different_luminance/sam_test_on_gabors_different_luminance_contour_plot_ppd_{ppd_number}_final_aim.json'
    ...
    for sam_model_index in tqdm(range(len(sam_model_name_list))):
        ...
        plot_L_b_matrix = np.array(plot_L_b_matrix_list[sam_model_index])
        ...
        # plt.tight_layout()
        plt.legend()
        plt.savefig(os.path.join(real_save_path, f'cos similarity - sam - ppd_{ppd_number}.png'),
