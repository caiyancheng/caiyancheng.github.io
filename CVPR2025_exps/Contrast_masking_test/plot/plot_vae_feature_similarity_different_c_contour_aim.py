...
json_data_path = rf'../test/new_data_logs/vae/different_c/vae_test_on_contrast_masking_different_c_contour_plot_ppd_{ppd}_final_aim.json'
...
backbone_name_list = plot_json_data['vae_model_name']
...
for backbone_index in tqdm(range(len(backbone_name_list))):
    ...
    real_save_path = os.path.join(save_root_path, backbone_name.split('/')[-1])
    ...
    # plt.show()
    plt.savefig(os.path.join(real_save_path, 'cos similarity - vae'), dpi=300, bbox_inches='tight',
                pad_inches=0.02)
    plt.close()
