...
json_data_path = rf'../test/new_data_logs/sam/different_c/sam_test_on_contrast_masking_different_c_contour_plot_ppd_{ppd}_final_aim.json'
...
for backbone_index in tqdm(range(len(backbone_name_list))):
    ...
    # plt.show()
    plt.savefig(os.path.join(real_save_path, 'cos similarity - sam'), dpi=300, bbox_inches='tight',
                pad_inches=0.02)
    plt.close()
