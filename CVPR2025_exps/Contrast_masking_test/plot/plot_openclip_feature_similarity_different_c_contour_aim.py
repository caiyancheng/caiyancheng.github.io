...
json_data_path = rf'../test/new_data_logs/openclip/different_c/openclip_test_on_contrast_masking_different_c_contour_plot_ppd_{ppd}_final_aim.json'
...
for clip_model_index in tqdm(range(len(clip_model_name_list))):
    ...
    diff_list = list(abs(contours.levels - aim_cos_similarity_list[clip_model_index]))
    ...
    # plt.show()
    plt.savefig(os.path.join(real_save_path, 'cos similarity - openclip'), dpi=300, bbox_inches='tight',
                pad_inches=0.02)
    plt.close()
