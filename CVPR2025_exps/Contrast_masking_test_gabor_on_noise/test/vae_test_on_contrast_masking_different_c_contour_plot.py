    ...
    for contrast_mask_index in range(len(contrast_mask_list)):
        ...
        for contrast_test_index in range(len(contrast_test_list)):
            ...
            df.to_csv(os.path.join(save_root_path,
                                   f'vae_test_on_contrast_masking_different_c_contour_plot_ppd_{default_ppd}_temporary.csv'),
                      index=False)
...
df = pd.DataFrame(csv_data)
df.to_csv(
    os.path.join(save_root_path, f'vae_test_on_contrast_masking_different_c_contour_plot_ppd_{default_ppd}_final.csv'),
    index=False)
with open(os.path.join(save_root_path,
                       f'vae_test_on_contrast_masking_different_c_contour_plot_ppd_{default_ppd}_final.json'),
          'w') as fp:
