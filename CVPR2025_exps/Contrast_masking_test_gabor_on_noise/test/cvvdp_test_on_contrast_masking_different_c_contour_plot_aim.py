...
Spearman_matrix_JOD = np.zeros([len(gt_x_mask_C), len(multiplier_list)])
for contrast_mask_index in tqdm(range(len(gt_x_mask_C))):
    ...
    for multiplier_index, multiplier_value in enumerate(multiplier_list):
        ...
        Spearman_matrix_JOD[contrast_mask_index, multiplier_index] = float(JOD)

json_file_name = r'new_data_logs\cvvdp\different_c/cvvdp_test_on_contrast_masking_different_c_contour_plot_ppd_60_final.json'
save_json_file_name = r'new_data_logs\cvvdp\different_c/cvvdp_test_on_contrast_masking_different_c_contour_plot_ppd_60_final_aim.json'
