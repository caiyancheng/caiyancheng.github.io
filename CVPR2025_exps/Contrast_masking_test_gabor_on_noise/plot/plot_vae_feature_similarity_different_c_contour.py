...
save_root_path = 'contour_plots/vae/different_c'
os.makedirs(save_root_path, exist_ok=True)

json_data_path = rf'../test/new_data_logs/vae/different_c/vae_test_on_contrast_masking_different_c_contour_plot_ppd_{ppd}_final.json'
...
vae_model_list = plot_json_data['vae_model_name']
...
for vae_model_index in tqdm(range(len(vae_model_list))):
    vae_model_name = vae_model_list[vae_model_index]
    ...
    plot_contrast_mask_matrix = np.array(plot_contrast_mask_matrix_list[vae_model_index])
    plot_contrast_test_matrix = np.array(plot_contrast_test_matrix_list[vae_model_index])
    ...
    plot_figure_name_list = [f'{vae_model_name_simple} - L1 similarity - final feature',
                             ...
                             f'{vae_model_name_simple} - cos similarity - final feature']
    ...
    for figure_index in range(len(plot_figure_name_list)):
        ...
        # plt.show()
        plt.savefig(os.path.join(real_save_path, plot_figure_name_list[figure_index])+'.png', dpi=300, bbox_inches='tight',
                    pad_inches=0.02)
        plt.close()
