...
save_root_path = 'contour_plots/sam2/different_c'
os.makedirs(save_root_path, exist_ok=True)

for ppd_index in range(len(ppd_list)):
    ppd_number = ppd_list[ppd_index]
    json_data_path = rf'../test/new_data_logs/sam2/different_c/sam2_test_on_contrast_masking_different_c_contour_plot_ppd_{ppd_number}_final.json'
    ...
    for sam_model_index in tqdm(range(len(sam_model_list))):
        ...
        for figure_index in range(len(plot_figure_name_list)):
            ...
            # plt.show()
            plt.savefig(os.path.join(real_save_path, plot_figure_name_list[figure_index] + f'_ppd_{ppd_number}.png'), dpi=300, bbox_inches='tight',
                        pad_inches=0.02)
            plt.close()
