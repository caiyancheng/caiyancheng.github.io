...
json_data_path = rf'../test/new_data_logs/openclip/different_rho/openclip_test_on_band_lim_noise_different_rho_band_contour_plot_ppd_{ppd}_final_aim.json'
...
for clip_model_index in tqdm(range(len(clip_model_name_list))):
    if clip_model_index == len(clip_model_name_list) - 1:
        X = 1
    ...
    aim_cos_similarity_nf = aim_cos_similarity_nf_list[clip_model_index]
    aim_cos_similarity_f = aim_cos_similarity_f_list[clip_model_index]
    Spearman_matrix_cos_nf = Spearman_matrix_cos_nf_list[clip_model_index]
    Spearman_matrix_cos_f = Spearman_matrix_cos_f_list[clip_model_index]
    ...
    plot_figure_name_list = ['cos similarity - openclip', 'cos similarity - openclip - fix_random_seed']
