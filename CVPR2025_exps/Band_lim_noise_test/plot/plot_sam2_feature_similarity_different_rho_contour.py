...
save_root_path = 'contour_plots/sam2/different_rho'
os.makedirs(save_root_path, exist_ok=True)

for ppd_index in range(len(ppd_list)):
    ppd_number = ppd_list[ppd_index]
    json_data_path = rf'../test/new_data_logs/sam2/different_rho/sam2_test_on_band_lim_noise_different_rho_band_contour_plot_ppd_{ppd_number}_final.json'
    ...
    plot_final_feature_L1_similarity_matrix_f_list = plot_json_data[
        'final_feature_L1_similarity_matrix_fix_random_seed']
    plot_final_feature_L2_similarity_matrix_f_list = plot_json_data[
        'final_feature_L2_similarity_matrix_fix_random_seed']
    plot_final_feature_cos_similarity_matrix_f_list = plot_json_data[
        'final_feature_cos_similarity_matrix_fix_random_seed']
    ...
    for sam_model_index in tqdm(range(len(sam_model_name_list))):
        ...
        plot_final_feature_L1_similarity_matrix_f = np.array(
            plot_final_feature_L1_similarity_matrix_f_list[sam_model_index])
        plot_final_feature_L2_similarity_matrix_f = np.array(
            plot_final_feature_L2_similarity_matrix_f_list[sam_model_index])
        plot_final_feature_cos_similarity_matrix_f = np.array(
            plot_final_feature_cos_similarity_matrix_f_list[sam_model_index])
        ...
        plot_figure_name_list = [f'{sam_model_name_simple} - L1 similarity',
                                 f'{sam_model_name_simple} - L2 similarity',
                                 f'{sam_model_name_simple} - cos similarity',
                                 f'{sam_model_name_simple} - L1 similarity - fix_random_seed',
                                 f'{sam_model_name_simple} - L2 similarity - fix_random_seed',
                                 f'{sam_model_name_simple} - cos similarity - fix_random_seed']
