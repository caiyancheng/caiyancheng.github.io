...
json_data_path = rf'../test/new_data_logs/openclip/different_rho/openclip_test_on_band_lim_noise_different_rho_band_contour_plot_ppd_{ppd}_final.json'
...
for clip_model_index in tqdm(range(len(clip_model_name_list))):
    ...
    plot_final_feature_L1_similarity_matrix_f = np.array(plot_final_feature_L1_similarity_matrix_f_list[clip_model_index])
    plot_final_feature_L2_similarity_matrix_f = np.array(plot_final_feature_L2_similarity_matrix_f_list[clip_model_index])
    plot_final_feature_cos_similarity_matrix_f = np.array(
        plot_final_feature_cos_similarity_matrix_f_list[clip_model_index])
    ...
    plot_figure_name_list = [f'{clip_model_name} - {clip_model_trainset} - L1 similarity',
                             ...
                             f'{clip_model_name} - {clip_model_trainset} - L1 similarity - fix_random_seed',
                             f'{clip_model_name} - {clip_model_trainset} - L2 similarity - fix_random_seed',
                             f'{clip_model_name} - {clip_model_trainset} - cos similarity - fix_random_seed']
