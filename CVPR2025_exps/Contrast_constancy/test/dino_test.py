...
# all_backbone_list = ['dino_xcit_small_12_p8', 'dino_xcit_medium_24_p16', 'dino_xcit_medium_24_p8', 'dino_resnet50']
...
def test_feature_cos_similarity(contrast, rho, backbone_model):
    ...
    T_vid_ct_feature = backbone_model(T_vid_ct)
    T_cos_similarity = float(F.cosine_similarity(R_vid_Con_0_ct_feature, T_vid_ct_feature).cpu())
    return T_cos_similarity
def T_optimize_target(contrast, rho, backbone_model, min_T_cos_similarity, aim_contrast_matching_scale):
    T_cos_similarity = test_feature_cos_similarity(contrast, rho, backbone_model)
...
for backbone_name in tqdm(all_backbone_list):
    ...
    backbone_model.eval()
    backbone_model.cuda()
    R_vid_Con_0_ct_feature = backbone_model(R_vid_Con_0_ct)
    R_vid_Con_1_ct_feature = backbone_model(R_vid_Con_1_ct)
    ...
    for reference_contrast_index in range(len(reference_contrast_list)):
        ...
        R_vid_ct_feature = backbone_model(R_vid_ct)
        ...
        for rho_test_index in range(len(rho_test_list)):
            ...
            T_vid_Con_1_ct_feature = backbone_model(T_vid_Con_1_ct)
            ...
            target_function = lambda test_contrast: T_optimize_target(
                contrast=test_contrast, rho=rho_test_value, backbone_model=backbone_model,
            ...
            df.to_csv(os.path.join(save_root_path, f'dino_test_ppd_{default_ppd}_temporary.csv'), index=False)
        json_plot_data[backbone_name][f'ref_contrast_{reference_contrast_value}_test_contrast_list'] = match_test_contrast_list

df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'dino_test_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'dino_test_ppd_{default_ppd}_final.json'), 'w') as fp:
