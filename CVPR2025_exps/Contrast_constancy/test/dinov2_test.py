...
for backbone_name in tqdm(all_backbone_list):
    ...
    for reference_contrast_index in range(len(reference_contrast_list)):
        ...
        for rho_test_index in range(len(rho_test_list)):
            ...
            df.to_csv(os.path.join(save_root_path, f'dinov2_test_ppd_{default_ppd}_temporary.csv'), index=False)
        json_plot_data[backbone_name][f'ref_contrast_{reference_contrast_value}_test_contrast_list'] = match_test_contrast_list

df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'dinov2_test_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'dinov2_test_ppd_{default_ppd}_final.json'), 'w') as fp:
