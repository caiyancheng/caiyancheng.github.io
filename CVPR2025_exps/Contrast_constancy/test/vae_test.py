...
json_plot_data = {}
json_plot_data['backbone_name_list'] = all_vae_model_list
...
def test_feature_cos_similarity(contrast, rho, vae_model):
    ...
    T_vid_ct_feature = vae_model.encode(norm_T_vid_ct).latent_dist.sample()
...
def T_optimize_target(contrast, rho, vae_model, min_T_cos_similarity, aim_contrast_matching_scale):
    T_cos_similarity = test_feature_cos_similarity(contrast, rho, vae_model)
...
for backbone_name in tqdm(all_vae_model_list):
    json_plot_data[backbone_name] = {}
    vae_model = AutoencoderKL.from_pretrained(backbone_name, subfolder="vae")
    R_vid_Con_0_ct_feature = vae_model.encode(norm_R_vid_Con_0_ct).latent_dist.sample()
    R_vid_Con_1_ct_feature = vae_model.encode(norm_R_vid_Con_1_ct).latent_dist.sample()
    ...
    for reference_contrast_index in range(len(reference_contrast_list)):
        ...
        R_vid_ct_feature = vae_model.encode(norm_R_vid_ct).latent_dist.sample()
        ...
        for rho_test_index in range(len(rho_test_list)):
            ...
            T_vid_Con_1_ct_feature = vae_model.encode(norm_T_vid_Con_1_ct).latent_dist.sample()
            ...
            target_function = lambda test_contrast: T_optimize_target(
                contrast=test_contrast, rho=rho_test_value, vae_model=vae_model,
            ...
            df.to_csv(os.path.join(save_root_path, f'vae_test_ppd_{default_ppd}_temporary.csv'), index=False)
        json_plot_data[backbone_name][f'ref_contrast_{reference_contrast_value}_test_contrast_list'] = match_test_contrast_list

df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'vae_test_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'vae_test_ppd_{default_ppd}_final.json'), 'w') as fp:
