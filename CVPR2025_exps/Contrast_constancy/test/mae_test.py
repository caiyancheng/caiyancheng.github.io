...
json_plot_data = {}
json_plot_data['backbone_name_list'] = all_mae_model_list
...
def test_feature_cos_similarity(contrast, rho, model, processor):
    ...
    inputs = processor(images=T_vid_ct, return_tensors="pt")
    inputs["noise"] = default_noise
    T_vid_ct_feature = model.vit(**inputs).last_hidden_state
...
def T_optimize_target(contrast, rho, model, processor, min_T_cos_similarity, aim_contrast_matching_scale):
    T_cos_similarity = test_feature_cos_similarity(contrast, rho, model, processor)
...
for mae_model_index in tqdm(range(len(all_mae_model_list))):
    ...
    inputs = processor(images=R_vid_Con_0_ct, return_tensors="pt")
    inputs["noise"] = default_noise
    R_vid_Con_0_ct_feature = model.vit(**inputs).last_hidden_state
    inputs = processor(images=R_vid_Con_1_ct, return_tensors="pt")
    inputs["noise"] = default_noise
    R_vid_Con_1_ct_feature = model.vit(**inputs).last_hidden_state
    min_R_cos_similarity = float(F.cosine_similarity(R_vid_Con_0_ct_feature.view(1, -1), R_vid_Con_1_ct_feature.view(1, -1)).cpu())
    for reference_contrast_index in range(len(reference_contrast_list)): #1
        ...
        inputs = processor(images=R_vid_ct, return_tensors="pt")
        inputs["noise"] = default_noise
        R_vid_ct_feature = model.vit(**inputs).last_hidden_state
        ...
        for rho_test_index in range(len(rho_test_list)): #4
            ...
            inputs = processor(images=T_vid_Con_1_ct, return_tensors="pt")
            inputs["noise"] = default_noise
            T_vid_Con_1_ct_feature = model.vit(**inputs).last_hidden_state
            ...
            target_function = lambda test_contrast: T_optimize_target(
                contrast=test_contrast, rho=rho_test_value, model=model, processor=processor,
            ...
            #     test_contrast_optimization_result = root_scalar(target_function, bracket=bounds, xtol=1e-5)
            #     test_contrast_optimization_value = test_contrast_optimization_result.root
            csv_data['backbone_name'].append(mae_model_name)
            ...
            df.to_csv(os.path.join(save_root_path, f'mae_test_ppd_{default_ppd}_temporary.csv'), index=False)
        json_plot_data[mae_model_name][f'ref_contrast_{reference_contrast_value}_test_contrast_list'] = match_test_contrast_list

df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'mae_test_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'mae_test_ppd_{default_ppd}_final.json'), 'w') as fp:
