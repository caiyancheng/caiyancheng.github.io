...
for backbone_name in tqdm(all_backbone_list):
    ...
    plot_T_freq_band_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_contrast_test_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_final_feature_L1_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_final_feature_L2_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_final_feature_cos_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_intermediate_feature_L1_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_intermediate_feature_L2_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    plot_intermediate_feature_cos_similarity_matrix = np.zeros([len(T_freq_band_list), len(contrast_test_list)])
    ...
    for T_freq_band_index in range(len(T_freq_band_list)):
        ...
        for contrast_test_index in range(len(contrast_test_list)):
            ...
            plot_T_freq_band_matrix[T_freq_band_index, contrast_test_index] = T_freq_band_value
            ...
            T_vid, R_vid = generate_contrast_masking_band_limit_noise(W=default_W, H=default_H,
                                                                      ...
                                                                      L_b=default_L_b,
                                                                      contrast_mask=default_contrast_mask,
            ...
            plot_final_feature_L1_similarity_matrix[T_freq_band_index, contrast_test_index] = L1_similarity
            plot_intermediate_feature_L1_similarity_matrix[T_freq_band_index, contrast_test_index] = L1_similarity_intermediate
            ...
            plot_final_feature_L2_similarity_matrix[T_freq_band_index, contrast_test_index] = L2_similarity
            plot_intermediate_feature_L2_similarity_matrix[T_freq_band_index, contrast_test_index] = L2_similarity_intermediate
            ...
            plot_final_feature_cos_similarity_matrix[T_freq_band_index, contrast_test_index] = cos_similarity
            plot_intermediate_feature_cos_similarity_matrix[T_freq_band_index, contrast_test_index] = cos_similarity_intermediate

            df = pd.DataFrame(csv_data)
            df.to_csv(os.path.join(save_root_path, f'dinov2_test_on_contrast_masking_different_rho_contour_plot_ppd_{default_ppd}_temporary.csv'), index=False)
    json_plot_data['contrast_test_matrix'].append(plot_contrast_test_matrix.tolist())
    json_plot_data['T_freq_band_matrix'].append(plot_T_freq_band_matrix.tolist())
...
df = pd.DataFrame(csv_data)
df.to_csv(os.path.join(save_root_path, f'dinov2_test_on_contrast_masking_different_rho_contour_plot_ppd_{default_ppd}_final.csv'), index=False)
with open(os.path.join(save_root_path, f'dinov2_test_on_contrast_masking_different_rho_contour_plot_ppd_{default_ppd}_final.json'), 'w') as fp:
